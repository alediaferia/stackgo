...
func (s *Stack) Push(elem ...interface{}) {
	...
	if s.size+len(elem) > s.capacity {
		pages_count := (s.size + len(elem) - s.capacity) / s.pageSize
		if (s.size+len(elem)-s.capacity)%s.pageSize != 0 {
			pages_count++
		}
		s.capacity += s.pageSize * pages_count
		for pages_count > 0 {
	...
	if s.offset == s.pageSize {
		s.offset = 0
		s.currentPageIndex++
		s.currentPage = s.pages[s.currentPageIndex]
	}
	available := s.pageSize - s.offset
	s.size += len(elem)
